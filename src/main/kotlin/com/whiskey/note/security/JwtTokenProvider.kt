...
class JwtTokenProvider(
    ...
    fun resolveToken(request: ServerRequest): String? {
        val bearerToken = request.headers().firstHeader(JwtProperties.HEADER_STRING).orEmpty()
    ...
    fun getAuthentication(token: String): Authentication {
        ...
        val email: String = (this.decodeJwtToken(token).body?.get("loginAccount") as String)
    ...
    fun extractToken(request: ServerWebExchange): String? =
        request.response.headers.getFirst("Authorization")

    fun extractToken(request: ServerRequest): String? =
